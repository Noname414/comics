...
這是四格漫畫生成器的網頁展示平台，提供美觀的界面來瀏覽和生成四格漫畫。所有生成的漫畫都儲存在 `docs/` 目錄下，可以通過網頁界面或 GitHub Pages 訪問。
...
docs/
├── 🌐 index.html              # 主要網頁界面
├── 📜 comic-api.js             # 前端 JavaScript 邏輯
├── 📊 comics_manifest.json     # 漫畫清單（自動生成）
├── 📚 README.md               # 本說明文檔
└── 📁 comic_output_*/         # 生成的漫畫資料夾
    ├── 🖼️ four_panel_comic.png   # 完整四格漫畫
    ├── 📄 comic_script.txt        # 漫畫腳本
